...
export const Rating: FC<RatingProps> = ({ commentsAvgRate, commentsCount, size = 'small', isCreateComment, setRate }) => {
		...
		<div className='flex'>
			{params.map(item => (
				<svg
					...
					fill="currentColor"
					aria-hidden="true"
					className={ classNames(sizeIcon, { 'text-gray-500': showRate < item.rate, 'text-amber-400': showRate >= item.rate, 'pointer-events-none': !isCreateComment }) }
				>
					<path
						fillRule="evenodd"
